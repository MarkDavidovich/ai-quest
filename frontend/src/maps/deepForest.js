export const createDeepForestLevel = ({ GRID_WIDTH, GRID_HEIGHT, toWorldKey }) => {
  ...
  const isWaterTile = (x, y) => DEEP_FOREST_WATER_REGIONS.some((region) => y >= region.top && y <= region.bottom && x >= region.left && x <= region.right);
  ...
  const isCartFootprintTile = (x, y) => DEEP_FOREST_CARTS.some(([cartRow, cartCol]) => x >= cartCol && x <= cartCol + 1 && y >= cartRow - 1 && y <= cartRow);
    ...
    interactive: Array(GRID_HEIGHT)
      ...
      .map((_, row) =>
        Array(GRID_WIDTH)
          .fill(null)
          .map((_, col) => {
            if (col === 0 && (row === 12 || row === 13)) return 4;
            const worldKey = toWorldKey(col, row);
            if (DEEP_FOREST_PATH_TILES.has(worldKey) || DEEP_FOREST_FIELD_TILES.has(worldKey)) return 0;
            return 3;
          }),
      ),
  };
